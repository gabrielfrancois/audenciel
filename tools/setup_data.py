...
def clean_time_string(time_str):
    ...
    if pd.isna(time_str): return time_str
    if time_str.endswith(':'): return time_str + '00'
    if len(time_str) == 13: return time_str + ':00:00'
    if len(time_str) == 16: return time_str + ':00'
    return time_str
...
def load_and_prepare(name, filename, house_id):
    ...
    type_col = 'type_defaut_fridge_freezzer' if 'type_defaut_fridge_freezzer' in df.columns else 'type_defaut_Fridge_Freezer'
    ...
    return df[['unix', 'time', 'aggregate', 'house_id', 'anomaly', 'anomaly_type', 'energy_gap']]
...
if __name__ == "__main__":
    ...
    labels_cols = ['anomaly', 'anomaly_type', 'energy_gap']
    ...
    print(f"Preparation complete: Train={len(train_normal)}, Test={len(test_df)}, Private={len(private_test_df)}")
